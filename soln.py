...
def inquirex(seat):
	...
	if not is_legal_seat(seat):
		...
		return
	if is_open(seat):
		print(seat," has not yet been sold")
		return
	sell_details = db[seat]
	print(seat,"has been sold to", sell_details[0],"on",sell_details[2],"for $%.2f." % sell_details[1])
...
def buy(seat):
	...
	if not is_open(seat):
		...
		print ("This seat is open and costs $", get_seat_cost(seat))
		response = input("Do you want to buy it?  (y/n)")
		if response == 'y':
			name = input("What is the purchaser's name? ")
			discount = 0
			response = input("Do you have a discount coupon? (y/n): ")
			if response == "y":
				response = input("What percentage is your discount?  (e.g.  10%)")
				if response[-1] == "%":
					response = response[0:-1]
				discount = float(response)
			seat_price = get_seat_cost(seat) - discount
			db[seat] = [name, seat_price, str(datetime.date.today())]
			print("Congratulations!  Pay by credit card now the sum of $%.2f" % seat_price)
			changed = True
		else:
...
def help():
	...
	print ("buy a5 -- try to buy seat a5")
	print ("inq a5 -- (inquire) what is the status of seat a5?")
	print ("refund a5 -- ask for a refund on a seat")
	print ("total -- how many seats are sold, how much money total")
	print ("view -- see the list of seats that are sold")
	print ("save -- save the database for later use")
	print ("load -- load the database that was saved")
	print ("    The name of the database file is ticketsystem.dat")
	print ("prompt >> -- change the prompt to whatever you want")
	print ("clear     -- clear the screen")
	print ("@any command   -- execute any Python system; just type @ first.")
	print ("*a system command -- execute an operating system command such as dir")
	print ("quit")
